...
from network.custom_functions.custom_fc import LinearSparse
from network.custom_functions.custom_sync_bn import SparseBatchNorm2d
from network.backbone.mobilenetv2 import InvertedResidual
...
def count_activation_size(net, input_size=(1, 3, 224, 224), require_backward=True, activation_bits=32):
	...
	# noinspection PyArgumentList
	def count_sparse_bn2d(m, x, _):
		# count activation size required by backward
		if m.weight is not None and m.weight.requires_grad:
			m.grad_activations = torch.Tensor([x[0].numel() * act_byte * (1 - m.masker.prune_ratio)])  # bytes
			m.grad_activations += torch.Tensor([x[0].numel() * (1 / 8)])  # bit map
		else:
			m.grad_activations = torch.Tensor([0])
		# temporary memory footprint required by inference
	...
	def add_hooks(m_):
		...
		if type(m_) in [nn.Conv1d, nn.Conv2d, nn.Conv3d]:
			...
			fn = count_bn
		elif type(m_) in [SparseBatchNorm2d]:
			fn = count_sparse_bn2d
	...
	for m in model.modules():
		...
		if (isinstance(m, InvertedResidual) and m.use_res_connect) or \
